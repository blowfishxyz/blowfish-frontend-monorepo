...
export class ScanMessageApi extends runtime.BaseAPI {
    ...
    async scanMessageEvmRaw(requestParameters: ScanMessageEvmOperationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<ScanMessageEvm200Response>> {
        ...
        if (requestParameters.chainFamily === null || requestParameters.chainFamily === undefined) {
            throw new runtime.RequiredError('chainFamily','Required parameter requestParameters.chainFamily was null or undefined when calling scanMessageEvm.');
        }

        if (requestParameters.chainNetwork === null || requestParameters.chainNetwork === undefined) {
            throw new runtime.RequiredError('chainNetwork','Required parameter requestParameters.chainNetwork was null or undefined when calling scanMessageEvm.');
        }
        ...
        const response = await this.request({
            path: `/{chain-family}/v0/{chain-network}/scan/message`.replace(`{${"chain-family"}}`, encodeURIComponent(String(requestParameters.chainFamily))).replace(`{${"chain-network"}}`, encodeURIComponent(String(requestParameters.chainNetwork))),
            method: 'POST',
...
export type ScanMessageEvmOperationChainFamilyEnum = typeof ScanMessageEvmOperationChainFamilyEnum[keyof typeof ScanMessageEvmOperationChainFamilyEnum];
/**
 * @export
 */
...
export type ScanMessageEvmOperationChainNetworkEnum = typeof ScanMessageEvmOperationChainNetworkEnum[keyof typeof ScanMessageEvmOperationChainNetworkEnum];
